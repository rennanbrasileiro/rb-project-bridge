...
contextBridge.exposeInMainWorld('rbBridge', {
  ...
  github: { status: () => ipcRenderer.invoke('github:status'), login: () => ipcRenderer.invoke('github:login'), logout: () => ipcRenderer.invoke('github:logout'), accounts: () => ipcRenderer.invoke('github:accounts') },
  migration: { start: (input) => ipcRenderer.invoke('migration:start', input), cancel: () => ipcRenderer.invoke('migration:cancel'), retryPublish: (jobRoot) => ipcRenderer.invoke('migration:retry-publish', jobRoot), history: () => ipcRenderer.invoke('migration:history'), clearHistory: () => ipcRenderer.invoke('migration:history-clear') },
