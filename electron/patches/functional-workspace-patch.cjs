...
const VERIFICATION_MIGRATION = '99999999999999_rb_functional_verification.sql';
...
function pick(data, names) {
  for (const name of names) if (data?.[name]) return data[name];
  return null;
}
...
async function safeCleanup(action) {
  try { await action(); } catch { /* Cleanup cannot overwrite the verification result. */ }
}

try {
  let statusRun = run(['status', '--output', 'json'], { capture: true, allowFailure: true });
  if (statusRun.status !== 0) {
    run(['start'], { timeout: 360000 });
    statusRun = run(['status', '--output', 'json'], { capture: true });
  }
  await check('migrations', 'Aplicar migrations em banco local limpo', async () => {
    run(['db', 'reset'], { timeout: 360000 });
    return 'Supabase local iniciado e migrations aplicadas.';
  });
  ...
  const firstEmail = 'rb.verify.' + suffix + '.one@example.com';
  const secondEmail = 'rb.verify.' + suffix + '.two@example.com';
  ...
  await check('create-users', 'Criar usuários temporários', async () => {
    ...
    secondUserId = two.data.user.id;
    return 'Dois usuários temporários criados.';
  });
  await check('login', 'Autenticar com e-mail e senha', async () => {
    const signed = await first.auth.signInWithPassword({ email: firstEmail, password });
    if (signed.error || !signed.data.session) throw signed.error || new Error('Session not created.');
    const secondSigned = await second.auth.signInWithPassword({ email: secondEmail, password });
    if (secondSigned.error || !secondSigned.data.session) throw secondSigned.error || new Error('Second session not created.');
    return 'Sessões reais criadas para dois usuários.';
  });
  await check('profiles', 'Confirmar criação automática de profiles', async () => {
    const profile = await first.from('profiles').select('id,role').eq('id', firstUserId).single();
    if (profile.error) throw profile.error;
    if (profile.data.role !== 'admin') throw new Error('Profile role was not copied from user metadata.');
    return 'Profile criado automaticamente após signup.';
  });
  await check('crud-create', 'Criar registro autenticado', async () => {
    const inserted = await first.from('rb_bridge_smoke').insert({ created_by_id: firstUserId, value: 'created' }).select().single();
    if (inserted.error) throw inserted.error;
    smokeId = inserted.data.id;
    return 'Registro criado pelo proprietário.';
  });
  await check('rls-anonymous', 'Bloquear leitura anônima', async () => {
    const result = await anonymous.from('rb_bridge_smoke').select('*').eq('id', smokeId);
    if (result.error) throw result.error;
    if (result.data.length !== 0) throw new Error('Anonymous client read protected data.');
    return 'Cliente anônimo não acessou o registro.';
  });
  await check('rls-other-user', 'Isolar dados entre usuários', async () => {
    const result = await second.from('rb_bridge_smoke').select('*').eq('id', smokeId);
    if (result.error) throw result.error;
    if (result.data.length !== 0) throw new Error('Second user read first user data.');
    const update = await second.from('rb_bridge_smoke').update({ value: 'intrusion' }).eq('id', smokeId).select();
    if (update.error) throw update.error;
    if (update.data.length !== 0) throw new Error('Second user updated first user data.');
    return 'Outro usuário não leu nem alterou o registro.';
  });
  await check('crud-update', 'Atualizar registro próprio', async () => {
    const updated = await first.from('rb_bridge_smoke').update({ value: 'updated' }).eq('id', smokeId).select().single();
    if (updated.error) throw updated.error;
    if (updated.data.value !== 'updated') throw new Error('Updated value was not persisted.');
    return 'Atualização persistida.';
  });
  await check('crud-delete', 'Excluir registro próprio', async () => {
    const removed = await first.from('rb_bridge_smoke').delete().eq('id', smokeId).select().single();
  ...
  if (admin) {
    if (smokeId) await safeCleanup(async () => { await admin.from('rb_bridge_smoke').delete().eq('id', smokeId); });
    if (firstUserId) await safeCleanup(async () => { await admin.auth.admin.deleteUser(firstUserId); });
    if (secondUserId) await safeCleanup(async () => { await admin.auth.admin.deleteUser(secondUserId); });
  }
}
`;
}

async function removeOldVerificationMigrations(migrationsDir) {
  const entries = await fs.readdir(migrationsDir, { withFileTypes: true }).catch(() => []);
  for (const entry of entries) {
    if (entry.isFile() && /_rb_functional_verification\.sql$/i.test(entry.name) && entry.name !== VERIFICATION_MIGRATION) {
      await fs.rm(path.join(migrationsDir, entry.name), { force: true });
    }
  }
}
...
async function enhanceWorkspace(root, report) {
  ...
  await removeOldVerificationMigrations(migrationsDir);
  const migrationPath = path.join(migrationsDir, VERIFICATION_MIGRATION);
...
module.exports = {
  ...
  verifierSource,
  removeOldVerificationMigrations,
  enhanceWorkspace,
};
