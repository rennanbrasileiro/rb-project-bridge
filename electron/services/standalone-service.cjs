...
class StandaloneService {
  ...
  async verify(root) {
    ...
    if (violations.length) throw new BridgeError('STANDALONE_GATE_FAILED', `A aplicação ainda possui dependências Base44 ou arquivos obrigatórios ausentes: ${violations.slice(0, 8).join(', ')}${violations.length > 8 ? '…' : ''}`, { violations });
