...
class MigrationService {
  ...
  async migrate(input) {
    ...
    try {
      ...
      report.githubRepository = { fullName: repository.full_name, cloneUrl: repository.clone_url, htmlUrl: repository.html_url, owner: repository.owner, private: repository.private, reused: resolved.reused, defaultBranch };
      ...
      report.status = bridgeError.code === 'MIGRATION_CANCELLED' ? 'cancelled' : 'failed'; report.finishedAt = new Date().toISOString(); bridgeError.details = { ...(bridgeError.details || {}), jobRoot, githubRepository: report.githubRepository }; report.error = { code: bridgeError.code, message: bridgeError.message, details: bridgeError.details };
  ...
  async retryPublish(jobRoot) {
    ...
    const repository = { full_name: report.githubRepository.fullName, clone_url: report.githubRepository.cloneUrl, html_url: report.githubRepository.htmlUrl, owner: report.githubRepository.owner, default_branch: report.githubRepository.defaultBranch || 'main' };
