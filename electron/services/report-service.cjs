...
const { writeOperationArtifacts } = require('./operation-summary-service.cjs');
...
class ReportService {
  ...
  async writeReport(directory, report, options = {}) {
    const context = getDeliveryContext(report.project?.id);
    if (context) {
      report.options = {
        ...(report.options || {}),
        deliveryPackage: context.deliveryPackage,
        targetProfile: context.targetProfile,
        clientName: context.clientName,
        deliveryOwner: context.deliveryOwner,
        migrationScope: context.migrationScope,
      };
    }

    report.readiness = assessMigrationReadiness(report);
    const delivery = await writeClientDeliveryPackage(directory, report);
    report.clientDelivery = {
      package: delivery.manifest.contractedPackage,
      target: delivery.manifest.targetProfile,
      acceptance: delivery.manifest.acceptance,
      directory: delivery.root,
      archive: report.clientDelivery?.archive || null,
    };

    const operationArtifacts = await writeOperationArtifacts(directory, report);
    report.operationSummary = operationArtifacts.summary;
    report.operationArtifacts = {
      operationPlanPath: operationArtifacts.operationPlanPath,
      extensionJsonPath: operationArtifacts.extensionJsonPath,
      extensionMarkdownPath: operationArtifacts.extensionMarkdownPath,
    };
    ...
    await fs.writeFile(markdownPath, mainMarkdown(report), 'utf8');

    let archive = null;
    if (!options.skipArchive) {
      archive = await createClientArchive(directory, report);
      if (archive) {
        report.clientDelivery.archive = archive;
        report.operationSummary = operationArtifacts.summary;
        report.operationSummary.artifacts.clientArchive = archive.path;
        await writeJson(jsonPath, report);
      }
    }
    ...
    return {
      ...
      clientDeliveryDirectory: delivery.root,
      clientDeliveryArchive: archive?.path || report.paths?.clientDeliveryArchive || null,
      operationPlanPath: operationArtifacts.operationPlanPath,
      pilotExtensionRequest: operationArtifacts.extensionJsonPath,
    };
  }
...
module.exports = { ReportService, mainMarkdown };
