...
class GitHubService {
  constructor({ toolchain, logger, emit, sessionDir }) {
  ...
  async runGh(args, options = {}) {
    ...
    return runProcess(gh, args, { timeoutMs: options.timeoutMs ?? 10 * 60 * 1000, input: options.input, env: this.sessionEnvironment(options.env), onOutput: options.onOutput ?? ((entry) => this.emit('github:output', entry)), signal: options.signal, captureSensitive: Boolean(options.captureSensitive) });
  }
  async runGit(args, options = {}) {
    await this.ensureSession();
    const git = await this.toolchain.getGit();
    return runProcess(git, args, { cwd: options.cwd, timeoutMs: options.timeoutMs ?? 10 * 60 * 1000, env: this.sessionEnvironment(options.env), onOutput: options.onOutput ?? ((entry) => this.emit('github:output', entry)), signal: options.signal });
  }
  async authStatus() {
    try { const result = await this.runGh(['auth', 'status', '--hostname', 'github.com'], { timeoutMs: 30_000 }); return { authenticated: true, output: result.stdout || result.stderr }; }
    catch (error) { return { authenticated: false, output: error.details?.stderr || error.message }; }
  }
  async login() { await this.runGh(['auth', 'login', '--hostname', 'github.com', '--git-protocol', 'https', '--web', '--skip-ssh-key'], { timeoutMs: 15 * 60 * 1000 }); return this.getAccounts(); }
  ...
  async getAccounts() {
    ...
    let orgs = [];
    try { const parsed = this.parseJson((await this.runGh(['api', 'user/orgs', '--paginate', '--slurp'], { timeoutMs: 60_000 })).stdout) || []; orgs = Array.isArray(parsed[0]) ? parsed.flat() : parsed; } catch (error) { this.logger.warn('github.organizations.unavailable', { message: error.message }); }
    if (!user?.login) throw new BridgeError('GITHUB_PROFILE_UNAVAILABLE', 'GitHub did not return the authenticated profile.');
    return { authenticated: true, user: { login: user.login, name: user.name || user.login, avatarUrl: user.avatar_url }, accounts: [{ login: user.login, type: 'user', label: user.name ? `${user.name} (${user.login})` : user.login }, ...orgs.map((org) => ({ login: org.login, type: 'organization', label: org.login }))] };
  }
  ...
  async createAskPass() {
    ...
    await fs.chmod(target, 0o700); return target;
  }
  async publish({ directory, repository, commitMessage, signal }) {
    ...
    const token = await this.getAuthenticationToken(); const askPass = await this.createAskPass();
    ...
    try {
      ...
      let pushError;
      for (let attempt = 1; attempt <= 3; attempt += 1) { try { await this.runGit(['push', '--set-upstream', 'origin', 'main'], { cwd: directory, timeoutMs: 20 * 60 * 1000, signal, env: authEnv }); pushError = null; break; } catch (error) { pushError = error; if (attempt < 3 && !signal?.aborted) await new Promise((resolve) => setTimeout(resolve, attempt * 2000)); } }
