...
class GitLabService {
  ...
  async authStatus() {
    try {
      const [user, tokenInfo] = await Promise.all([
        this.request('/user'),
        this.request('/personal_access_tokens/self', { allow404: true }).catch(() => null),
      ]);
      ...
      return { authenticated: false, scopes: [], missingDeliveryScopes: [...REQUIRED_SCOPES], deliveryReady: false, error: error.message };
    }
  }
  ...
  async ensureDeliveryScopes() {
    ...
    if (status.scopes.length && status.missingDeliveryScopes.length) {
      throw new BridgeError('GITLAB_SCOPE_REQUIRED', `O token GitLab precisa dos escopos: ${status.missingDeliveryScopes.join(', ')}.`, { scopes: status.scopes, requiredScopes: REQUIRED_SCOPES });
    }
    return status;
  }
  ...
  async runGit(args, options = {}) {
    ...
    return runProcess(git, args, {
      ...
      env: options.env,
      onOutput: options.onOutput ?? ((entry) => this.emit('gitlab:output', entry)),
      signal: options.signal,
    });
  }

  async repositoryExists(owner, name) {
    return Boolean(await this.request(`/projects/${encodeProject(projectPath(owner, name))}`, { allow404: true }));
  }
  ...
  async createRepository({ owner, name, description }, options = {}) {
    ...
    const project = await this.request('/projects', {
      method: 'POST',
      signal: options.signal,
      body: { name, path: name, namespace_id: namespace.id, visibility: 'private', description: description || '' },
    });
    return normalizeRepository(project);
  }
  ...
  async ensurePrivate(owner, name, options = {}) {
    ...
    const project = await this.request(`/projects/${repository.id}`, { method: 'PUT', signal: options.signal, body: { visibility: 'private' } });
    return normalizeRepository(project);
  }
  ...
  async getBranch(repository, branch, options = {}) {
    return this.request(`/projects/${repository.id}/repository/branches/${encodeURIComponent(branch)}`, { signal: options.signal, allow404: true });
  }

  async preserveBranch(repository, branch, options = {}) {
    const current = await this.getBranch(repository, branch, options);
    const sha = current?.commit?.id;
    if (!sha) return null;
    ...
    this.emit('migration:progress', { step: 'gitlab-snapshot', status: 'complete', message: `Branch ${branch} preservada em ${backupBranch} (${sha.slice(0, 7)}).` });
  ...
  async readRepositoryJson(repository, filePath, branches = ['main', 'base44-source']) {
    for (const branch of [...new Set(branches)]) {
      ...
      const target = `/projects/${repository.id}/repository/files/${encodedPath}/raw?ref=${encodeURIComponent(branch)}`;
      try {
        const token = await this.getToken();
        ...
        this.logger?.warn?.('gitlab.manifest.read.failed', { repository: repository.full_name, branch, filePath, message: error.message });
      }
    }
    return null;
  }
  ...
  async inspectSourceStatus({ owner, name, project }) {
    ...
    const latestCommitAt = latest?.committed_date || latest?.created_at || null;
    ...
    const gitChanged = Boolean(deliveredAt && latestCommitAt && new Date(latestCommitAt).getTime() > new Date(deliveredAt).getTime() + 120000);
    const snapshotOnly = Boolean(source && source.branch !== defaultBranch && !deliveredAt);
    return {
      exists: true,
      repository: { name: repository.name, fullName: repository.full_name, private: repository.private, defaultBranch, htmlUrl: repository.html_url, updatedAt: repository.updated_at },
      source: source ? { branch: source.branch, ...source.data } : null,
      currentBase44UpdatedAt: currentUpdatedAt,
      latestCommitAt,
      base44Changed,
      githubChanged: gitChanged,
      snapshotOnly,
      status: !source ? 'unlinked' : snapshotOnly ? 'snapshot-only' : base44Changed ? (gitChanged ? 'both-changed' : 'base44-newer') : gitChanged ? 'github-newer' : 'in-sync',
    };
  }

  async createAskPass() {
    await fs.mkdir(this.sessionDir, { recursive: true });
    const target = path.join(this.sessionDir, process.platform === 'win32' ? 'gitlab-askpass.cmd' : 'gitlab-askpass.sh');
    if (process.platform === 'win32') {
      await fs.writeFile(target, ['@echo off', 'echo %~1 | findstr /I "Username" >nul', 'if %errorlevel%==0 (', '  echo oauth2', ') else (', '  echo %RB_BRIDGE_GITLAB_TOKEN%', ')', ''].join('\r\n'), { encoding: 'utf8', mode: 0o700 });
    } else {
  ...
  async authEnvironment() {
    const token = await this.getToken();
    const askPass = await this.createAskPass();
    return { GIT_ASKPASS: askPass, GIT_TERMINAL_PROMPT: '0', RB_BRIDGE_GITLAB_TOKEN: token };
  }
  ...
  async cloneBase44Source({ owner, name, destination }, options = {}) {
    ...
    const env = await this.authEnvironment();
    try {
      for (const branch of [...new Set(branches)]) {
        if (!(await this.getBranch(repository, branch, options))) continue;
        ...
        try {
          ...
          const sha = (await this.runGit(['rev-parse', 'HEAD'], { cwd: destination, signal: options.signal, env })).stdout.trim();
          ...
          await fs.rm(destination, { recursive: true, force: true });
          this.logger?.warn?.('gitlab.source.clone.failed', { repository: repository.full_name, branch, message: error.message });
        }
      }
  ...
  async publish({ directory, repository, commitMessage, signal, branch = 'main', force = false }) {
    this.emit('migration:progress', { step: branch === (repository.default_branch || 'main') ? 'gitlab-publish' : 'gitlab-snapshot', status: 'running', message: `Publicando ${branch} em ${repository.full_name}...` });
    await this.ensureDeliveryScopes();
    await fs.rm(path.join(directory, '.git'), { recursive: true, force: true });
    const env = await this.authEnvironment();
    try {
      await this.runGit(['init', '-b', branch], { cwd: directory, signal, env });
      const login = repository.owner?.login || repository.full_name.split('/')[0];
      await this.runGit(['config', 'user.name', 'RB Project Bridge'], { cwd: directory, signal, env });
      await this.runGit(['config', 'user.email', `bridge@${String(login).replace(/[^a-z0-9.-]/gi, '-')}.local`], { cwd: directory, signal, env });
      await this.runGit(['add', '--all'], { cwd: directory, signal, env });
      await this.runGit(['commit', '-m', commitMessage || 'RB Project Bridge migration'], { cwd: directory, signal, env });
      await this.runGit(['remote', 'add', 'origin', repository.clone_url], { cwd: directory, signal, env });
      const push = ['push', '--set-upstream', 'origin', branch];
      if (force) push.splice(1, 0, '--force');
      await this.runGit(push, { cwd: directory, signal, env, timeoutMs: 20 * 60 * 1000 });
      const sha = (await this.runGit(['rev-parse', 'HEAD'], { cwd: directory, signal, env })).stdout.trim();
      if (branch === 'main') {
        await this.request(`/projects/${repository.id}`, { method: 'PUT', signal, body: { default_branch: 'main' } }).catch((error) => {
          this.logger?.warn?.('gitlab.default_branch.update.failed', { repository: repository.full_name, message: error.message });
        });
      }
      return { sha, url: repository.html_url, fullName: repository.full_name, branch, provider: 'gitlab' };
    } finally { delete env.RB_BRIDGE_GITLAB_TOKEN; }
  }

  async createPullRequest(repository, { head, base, title, body }, options = {}) {
    const merge = await this.request(`/projects/${repository.id}/merge_requests`, {
      method: 'POST', signal: options.signal,
      body: { source_branch: head, target_branch: base, title, description: body || '', remove_source_branch: false },
    });
